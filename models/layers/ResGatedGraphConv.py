...
from torch.nn import Parameter, LeakyReLU, ModuleList
...
class ResGatedGraphConv(MessagePassing):
    ...
    Args:
        ...
        act (callable, optional): Gating function :math:`\sigma`.
            (default: :meth:`torch.nn.LeakyReLU()`)
    ...
    def __init__(
        ...
        out_channels: int,
        act: Optional[Callable] = LeakyReLU(),
    ...
    def forward(
        ...
        #                  edge_attr: OptTensor)
        msg = self.propagate(edge_index, k=k, q=q, v=v, edge_attr=edge_attr)
        out = self.msg_norm(x[0], msg)
    ...
    def message(self, k_i: Tensor, q_j: Tensor, v_j: Tensor,
        ...
        print(edge_attr.size(), self.edge_dim)
